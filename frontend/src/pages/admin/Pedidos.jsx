...
const initialForm = {
  usuario: '',
  productos: '', // IDs o nombres separados por coma
  total: '',
  estado: 'pendiente',
  direccionEntrega: '',
  fechaPedido: '',
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-8">
        <h2 className="text-lg font-bold mb-6">Nuevo Pedido</h2>
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              <label className="block text-xs font-bold mb-1">Usuario</label>
              <input name="usuario" value={form.usuario} onChange={handleChange} className="w-full border rounded px-2 py-1" placeholder="ID o nombre de usuario" required />
            </div>
            <div>
              <label className="block text-xs font-bold mb-1">Productos</label>
              <input name="productos" value={form.productos} onChange={handleChange} className="w-full border rounded px-2 py-1" placeholder="IDs o nombres separados por coma" required />
            </div>
            <div>
              <label className="block text-xs font-bold mb-1">Total</label>
              <input name="total" type="number" value={form.total} onChange={handleChange} className="w-full border rounded px-2 py-1" placeholder="Total" required />
            </div>
            <div>
            ...
            <div className="col-span-2">
              <label className="block text-xs font-bold mb-1">Fecha de Pedido</label>
              <input name="fechaPedido" type="date" value={form.fechaPedido} onChange={handleChange} className="w-full border rounded px-2 py-1" />
            </div>
          </div>
